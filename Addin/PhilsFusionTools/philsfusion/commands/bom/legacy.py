...
from .domain import (
    bom_row_sort_key as _domain_bom_row_sort_key,
)
from .domain import (
...
_SETTINGS_BRIDGE = BomSettingsBridge(
    SettingsStore(_DOCUMENTS / "PhilsFusionTools" / "settings.json"),
    _DOCUMENTS / "PhilsBom" / "PhilsBom-Settings.plist",
)


def _log_path():
    try:
        base = Path(os.path.expanduser("~/Documents")) / "PhilsFusionTools" / "logs"
        base.mkdir(parents=True, exist_ok=True)
        return str(base / "bom.log")
    except Exception:
        return None
...
def SplitNameAndProfile(name):
    return split_name_and_profile(name)
...
def SettingsLoad():
    normalized = _normalize_settings(_SETTINGS_BRIDGE.load_legacy())
    _log(
...
def SettingsSave(appSettingsDictionary):
    ...
    _SETTINGS_BRIDGE.save_legacy(settings)
    _log("SettingsSave: wrote unified settings.json")
...
def GetAppSaveFilename(filenameExtension):
    try:
        appSaveLocation = Path(os.path.expanduser("~/Documents")) / "PhilsFusionTools"
        appSaveLocation.mkdir(parents=True, exist_ok=True)
        return str(appSaveLocation / f"{COMMAND_ID}{filenameExtension}")
    except Exception:
        if UI:
            UI.messageBox("ERROR:\n{}".format(traceback.format_exc()))
        return ""
...
def GetCustomItemNumberSequence(template):
    yield from item_number_sequence(template)
...
def _bom_row_sort_key(item):
    return _domain_bom_row_sort_key(item)
