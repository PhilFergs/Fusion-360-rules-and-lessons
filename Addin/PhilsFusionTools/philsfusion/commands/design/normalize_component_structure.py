        ...
        for i in range(occs.count):
            ...
            if parent_occurrence:
                try:
                    ...
                    logger.log(
                        "NORMALIZE: child occurrence context failed "
...
def _add_named_root_occurrence(design, target_name, reason):
    ...
    try:
        ...
        logger.log(
            "NORMALIZE: named root occurrence aborted name blocked "
        ...
        logger.log(
            "NORMALIZE: add named root occurrence failed "
        ...
        if assembly_context:
            try:
                ...
                if moved_occ:
                    ...
                    logger.log(
                        "NORMALIZE: moved named rebuilt wrapper into context "
                ...
                logger.log(
                    "NORMALIZE: rebuilt wrapper move into context failed "
...
def _rebuild_mixed_wrapper(design, comp, parent_occurrence, wrapper_name):
    ...
    for child_occ in child_occurrences:
        try:
            ...
            if moved:
                ...
                moved_count += 1
                logger.log(
                    "NORMALIZE: moved child into rebuilt wrapper "
                    + f"child='{child_name}' wrapper='{_occurrence_name(wrapper_occurrence)}'"
                )
            else:
                logger.log(
                    "NORMALIZE: move child into rebuilt wrapper returned null "
                    + f"child='{child_name}' wrapper='{_occurrence_name(wrapper_occurrence)}'"
                )
        except Exception as ex:
            logger.log(
                "NORMALIZE: move child into rebuilt wrapper failed "
                + f"child='{_occurrence_name(child_occ)}' "
    ...
    if not deleted_old:
        logger.log(
            "NORMALIZE: rebuilt wrapper kept but old wrapper could not be deleted "
...
def _move_body_to_new_child_component(design, parent_comp, body, parent_occurrence, target_component_name):
    ...
    try:
        ...
        if parent_occurrence:
            ...
            logger.log(
                "NORMALIZE: moved named child into parent "
...
def _execute():
    ...
    for comp_context in components_after:
        ...
        if _rebuild_mixed_wrapper(design, comp, parent_occurrence, wrapper_name):
            ...
            logger.log(
                "NORMALIZE: parent wrapper rebuilt "
